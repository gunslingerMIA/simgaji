...
@section('title', $titleJenis . ' PPPK')
@section('page_title', $titleJenis . ' PPPK')
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-0 d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        <div class="d-flex align-items-center gap-2 mb-1">
                            ...
                            <h4 class="mb-0 text-dark fw-bold">PPPK - Pencairan {{ str_pad($bulanCair, 2, '0', STR_PAD_LEFT) }}/{{ $tahunCair }}</h4>
                        </div>
                        <p class="text-muted small mb-0">
                            Menampilkan data daftar pembayaran {{ strtolower($titleJenis) }} PPPK (Gaji Pokok & Tunjangan Jabatan proporsional masa kerja).
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        @if(count($gajiTambahan) > 0)
                            ...
                            @endif
                        @endif
                        <a href="{{ route('gaji-tambahan-pppk.create', ['jenis' => $jenis]) }}" class="btn btn-primary fw-semibold">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex flex-wrap justify-content-between align-items-center border-bottom pb-3 mb-4 gap-3">
                        <ul class="nav nav-pills">
                            <li class="nav-item me-2">
                                <a class="nav-link {{ $jenis === 'thr' ? 'active bg-primary' : 'bg-light text-dark border' }} fw-semibold px-4" href="{{ route('gaji-tambahan-pppk.index', ['jenis' => 'thr', 'tahun_cair' => $tahunCair]) }}">
                                    <i class="fa-solid fa-gift me-1"></i> Gaji 14 (THR)
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link {{ $jenis === 'gaji_13' ? 'active bg-success' : 'bg-light text-dark border' }} fw-semibold px-4" href="{{ route('gaji-tambahan-pppk.index', ['jenis' => 'gaji_13', 'tahun_cair' => $tahunCair]) }}">
                        ...
                        <div class="btn-group">
                            ...
                            <a href="{{ route('gaji-tambahan-pppk.index', ['jenis' => $jenis, 'tahun_cair' => $tahunCair]) }}" class="btn btn-primary active">
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="card bg-light border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="text-muted small fw-semibold">Jumlah Pegawai PPPK</div>
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="card bg-light border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="text-muted small fw-semibold">Total Potongan PPh (TER)</div>
                    ...
                    @if(count($gajiTambahan) > 0)
                        @if($isLocked)
                            <form action="{{ route('gaji-tambahan-pppk.unlock') }}" method="POST" id="form-unlock" class="d-none">
                                @csrf
                                <input type="hidden" name="jenis" value="{{ $jenis }}">
                                <input type="hidden" name="tahun_cair" value="{{ $tahunCair }}">
                            </form>
                        @else
                            <form action="{{ route('gaji-tambahan-pppk.lock') }}" method="POST" id="form-lock" class="d-none">
                                @csrf
                                <input type="hidden" name="jenis" value="{{ $jenis }}">
                                <input type="hidden" name="tahun_cair" value="{{ $tahunCair }}">
                            </form>
                        ...
                        <table class="table table-hover table-bordered align-middle" style="font-size: 0.85em; white-space: nowrap;">
                            <thead class="table-light text-center align-middle">
                                <tr>
                                    ...
                                    <th rowspan="2">Gol</th>
                                    <th rowspan="2">TMT / Masa Kerja</th>
                                    <th colspan="9">Komponen Penghasilan</th>
                                ...
                                @forelse($gajiTambahan as $index => $gaji)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if($gaji->masa_kerja_bulan < 12)
                                                <span class="badge bg-warning text-dark fw-bold" title="Proporsional {{ $gaji->masa_kerja_bulan }}/12">
                                                    {{ $gaji->masa_kerja_bulan }} Bln ({{ $gaji->persen_proporsional }}%)
                                                </span>
                                            @else
                                            ...
                                            @endif
                                            @if($gaji->tmt)
                                                <div class="text-muted" style="font-size: 0.75em;">TMT: {{ $gaji->tmt->format('d/m/Y') }}</div>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-end fw-semibold">
                                            ...
                                            @if($gaji->gaji_pokok_dasar > $gaji->gaji_pokok)
                                                <div class="text-muted" style="font-size: 0.75em;" title="Gapok Penuh: Rp {{ number_format($gaji->gaji_pokok_dasar, 0, ',', '.') }}">
                                                    <del>Rp {{ number_format($gaji->gaji_pokok_dasar, 0, ',', '.') }}</del>
                                                </div>
                                            @endif
                                        </td>
                                            ...
                                            @if(!$isLocked)
                                                <div class="btn-group btn-group-sm">
                                                    <button type="button" class="btn btn-outline-primary btn-edit"
                                                        data-id="{{ $gaji->id }}"
                                                        data-nama="{{ $gaji->nama }}"
                                                        data-masakerja="{{ $gaji->masa_kerja_bulan }}"
                                                        data-gapokdasar="{{ $gaji->gaji_pokok_dasar > 0 ? $gaji->gaji_pokok_dasar : $gaji->gaji_pokok }}"
                                                    ...
                                                    <form id="form-delete-{{ $gaji->id }}" action="{{ route('gaji-tambahan-pppk.destroy', $gaji->id) }}" method="POST" class="d-none">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="19" class="text-center py-5 text-muted">
                                            ...
                                            Belum ada data {{ $shortLabel }} PPPK untuk periode pencairan ini.<br>
                                            <a href="{{ route('gaji-tambahan-pppk.create', ['jenis' => $jenis]) }}" class="btn btn-sm btn-primary mt-2">
...
<!-- Modal Edit Tunjangan Manual PPPK -->
        ...
        <form id="formEdit" method="POST">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="modalEditLabel"><i class="fa-solid fa-pen-to-square text-primary me-2"></i>Sesuaikan Data PPPK: <span id="editNama" class="text-primary"></span></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="editMasaKerja" class="form-label fw-semibold">Masa Kerja (Bulan, Max 12)</label>
                        <div class="input-group">
                            <input type="number" min="1" max="12" class="form-control" id="editMasaKerja" name="masa_kerja_bulan" required>
                            <span class="input-group-text">/ 12 Bulan</span>
                        </div>
                        <div class="form-text small text-muted">Masa kerja akan otomatis menghitung proporsional Gaji Pokok.</div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="editPph" class="form-label fw-semibold">Potongan PPh 21 (TER)</label>
                        <input type="number" step="any" class="form-control" id="editPph" name="potongan_pph" required>
                    </div>
                </div>
...
document.querySelectorAll('.btn-edit').forEach(button => {
    button.addEventListener('click', function() {
        ...
        document.getElementById('editMasaKerja').value = masakerja;
        ...
        document.getElementById('formEdit').action = `/gaji-tambahan-pppk/${id}`;
...
document.querySelectorAll('.btn-delete').forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            html: `Apakah Anda yakin ingin menghapus <b>${nama}</b> dari daftar ${label} PPPK periode ini?<br><small class="text-muted">(Contoh: Pegawai mutasi keluar / pindah OPD sebelum bulan pencairan)</small>`,
            icon: 'warning',

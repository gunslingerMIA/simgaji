...
@section('title', 'Gaji Induk PNS')
@section('page_title', 'Gaji Induk PNS')
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body">
                                ...
                                <div class="card bg-light border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="fs-5 fw-bold text-dark">{{ count($gajiPns) }} Orang</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card bg-light border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="fs-5 fw-bold text-dark">Rp {{ number_format($gajiPns->sum('kotor_resmi'), 0, ',', '.') }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card bg-light border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="fs-5 fw-bold text-dark">Rp {{ number_format($gajiPns->sum('jumlah_potongan'), 0, ',', '.') }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card bg-light border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <div class="fs-5 fw-bold text-success">Rp {{ number_format($gajiPns->sum('bersih_resmi'), 0, ',', '.') }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <form action="{{ route('gaji-induk-pns.index') }}" method="GET" class="mb-4">
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-6 text-end">
                                @if(count($gajiPns) > 0)
                                    @php $isLocked = $gajiPns->first()->is_locked; @endphp
                                    @if($isLocked)
                    ...
                    @if(count($gajiPns) > 0)
                        @if($isLocked)
                            <form action="{{ route('gaji-induk-pns.unlock') }}" method="POST" id="form-unlock" class="d-none">
                                @csrf
                                <input type="hidden" name="bulan" value="{{ $bulan }}">
                                <input type="hidden" name="tahun" value="{{ $tahun }}">
                            </form>
                        @else
                            <form action="{{ route('gaji-induk-pns.lock') }}" method="POST" id="form-lock" class="d-none">
                                @csrf
                                <input type="hidden" name="bulan" value="{{ $bulan }}">
                                <input type="hidden" name="tahun" value="{{ $tahun }}">
                            </form>
                        ...
                        <table class="table table-hover table-bordered align-middle" style="font-size: 0.85em; white-space: nowrap;">
                            ...
                            <tbody>
                                @forelse($gajiPns as $index => $gaji)
                                    <tr>
                                        ...
                                        <td class="text-end">{{ number_format($gaji->tunjangan_pph, 0, ',', '.') }}</td>
                                        ...
                                        <td class="text-end">{{ number_format($gaji->potongan_iwp_8, 0, ',', '.') }}</td>
                            ...
                            <tfoot class="table-light text-end fw-bold">
                                <tr>
                                    ...
                                    <td>{{ number_format($gajiPns->sum('gaji_pokok'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_suami_istri'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_anak'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_jabatan'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_fungsional'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_umum'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_beras'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_bpjs'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_jkk'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_jkm'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_pph'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('tunjangan_pembulatan'), 0, ',', '.') }}</td>
                                    <td class="bg-primary bg-opacity-10 text-primary">{{ number_format($gajiPns->sum('kotor_resmi'), 0, ',', '.') }}</td>
                                    
                                    <td>{{ number_format($gajiPns->sum('potongan_iwp_1'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('potongan_iwp_8'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('potongan_bpjs'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('potongan_jkk'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('potongan_jkm'), 0, ',', '.') }}</td>
                                    <td>{{ number_format($gajiPns->sum('potongan_pph'), 0, ',', '.') }}</td>
                                    <td class="bg-danger bg-opacity-10 text-danger">{{ number_format($gajiPns->sum('jumlah_potongan'), 0, ',', '.') }}</td>
                                    
                                    <td class="bg-success bg-opacity-10 text-success fs-6">{{ number_format($gajiPns->sum('bersih_resmi'), 0, ',', '.') }}</td>
                                    <td></td>
                                </tr>
                            </tfoot>
                            @endif
        ...
        <div class="modal-content">
            ...
            <form id="editTunjanganForm" method="POST" action="">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold text-danger">Tunjangan PPh (Rp)</label>
                        <input type="number" name="tunjangan_pph" id="edit_tunj_pph" class="form-control" required min="0">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                form.setAttribute('action', `/gaji-induk-pns/${id}`);
                modal.show();
            });
        });
